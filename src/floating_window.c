...
#define RAYGUI_WINDOW_CLOSEBUTTON_SIZE 18
    ...
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && !*moving && !*resizing) {
        ...
        Rectangle title_collision_rect = {position->x,
                                          position->y,
                                          size->x -
                                              (float)(RAYGUI_WINDOW_CLOSEBUTTON_SIZE + close_title_size_delta_half),
    ...
    if (*moving) {
        ...
        if (IsMouseButtonReleased(MOUSE_LEFT_BUTTON)) {
            *moving = false;
            ...
            else if (position->x > (float)GetScreenWidth() - size->x)
                position->x = (float)GetScreenWidth() - size->x;
            if (position->y < 0) position->y = 0;
            else if (position->y > (float)GetScreenHeight())
                position->y = (float)GetScreenHeight() - RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT;
        }
        ...
        else if (size->x > (float)GetScreenWidth())
            size->x = (float)GetScreenWidth();
        if (size->y < 100) size->y = 100;
        else if (size->y > (float)GetScreenHeight())
    ...
    if (*minimized) {
        ...
        if (GuiButton(
                (Rectangle){position->x + size->x - RAYGUI_WINDOW_CLOSEBUTTON_SIZE - (float)close_title_size_delta_half,
                            position->y + (float)close_title_size_delta_half,
        ...
        *minimized = GuiWindowBox((Rectangle){position->x, position->y, size->x, size->y}, title);
        ...
        if (draw_content != NULL) {
            Rectangle scissor = {0};
            GuiScrollPanel((Rectangle){position->x,
        ...
        GuiDrawIcon(71, (int)(position->x + size->x - 20), (int)(position->y + size->y - 20), 1, WHITE);
    }
}
...
//         GuiWindowFloating(&window2_position,
//                           &window2_size,
//                           &minimized2,
//                           &moving2,
//                           &resizing2,
