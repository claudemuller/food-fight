...
void GuiWindowFileDialog(GuiWindowFileDialogState* state)
{
    if (state->windowActive) {
        ...
        if (state->supportDrag) {
            ...
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                ...
                if (CheckCollisionPointRec(mousePosition,
                                           (Rectangle){state->windowBounds.x,
        ...
        if (GuiButton(
                (Rectangle){
                    state->windowBounds.x + state->windowBounds.width - 48, state->windowBounds.y + 24 + 12, 40, 24},
                "< ..")) {
...
#if defined(USE_CUSTOM_LISTVIEW_FILEINFO)
        state->filesListActive = GuiListViewFiles((Rectangle){state->position.x + 8,
                                                              state->position.y + 48 + 20,
            ...
            if (DirectoryExists(TextFormat("%s/%s", state->dirPathText, state->fileNameText))) {
                if (TextIsEqual(state->fileNameText, ".."))
                    ...
                    strcpy(state->dirPathText,
                           TextFormat("%s/%s",
                                      (strcmp(state->dirPathText, "/") == 0) ? "" : state->dirPathText,
                                      state->fileNameText));
        ...
        //--------------------------------------------------------------------------------------
        GuiLabel(
            (Rectangle){state->windowBounds.x + 8, state->windowBounds.y + state->windowBounds.height - 68, 60, 24},
            "File name:");
        if (GuiTextBox((Rectangle){state->windowBounds.x + 72,
        ...
        GuiLabel(
            (Rectangle){
                state->windowBounds.x + 8, state->windowBounds.y + state->windowBounds.height - 24 - 12, 68, 24},
            "File filter:");
...
static void ReloadDirectoryFiles(GuiWindowFileDialogState* state)
{
    ...
    state->dirFiles =
        LoadDirectoryFilesEx(state->dirPathText, (state->filterExt[0] == '\0') ? NULL : state->filterExt, false);
    state->itemFocused = 0;
    ...
    for (int i = 0; i < visibleItems; i++) {
        if (state == GUI_STATE_DISABLED) {
            if ((startIndex + i) == itemSelected) {
                ...
                DrawRectangleLinesEx(itemBounds,
